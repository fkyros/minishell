...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/06/01 13:59:06 by gade-oli          #+#    #+#             */
/*   Updated: 2025/06/01 14:23:41 by gade-oli         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int						print_path_error(char *path,
							t_parse_result *result, int i);
int						is_operator(const char *token);
char					*expand_variable(const char *line,
							int *i, t_mini *mini);

// QUOTE PARSING
int						quote_state_after(int state, char quote);
void					append_to_buf(char *buf, size_t *pos,
...
char					**add_var_to_env(char **our_env, char *name,
							char *value);
...
char					*extract_raw_heredoc_delim(const char *input,
							const char *eof_clean);
char					*expand_line(const char *line, t_mini *mini);
void					print_heredoc_warning(const char *eof);
void					write_heredoc_line(char *line, int expand,
							int fd, t_mini *mini);
int						heredoc_readline(const char *eof, int expand,
							int fd, t_mini *mini);
int						heredoc_fork_error(char *delim, int fd0, int fd1);
void					heredoc_child_process(char *delim, int expand,
							int write_fd, t_mini *mini);
int						heredoc_parent_after(pid_t pid, int read_fd,
							t_command *cmd);
...
// REDIRECTION PARSING & AUX
int						check_unclosed_quotes(const char *str);
int						count_tokens(const char *str);
int						is_whitespace(char c);
int						is_quote(char c);
void					skip_quoted_section(const char *str, int *index,
							char quote_char);
...
void					init_command(t_command *cmd,
							char **argv_start, int is_first);
int						fill_command(char **args, int *i,
							t_command *cmd, const char *input);
...
char					**parse_command(const char *cmd,
							int *token_count, t_mini *mini);
t_parse_result			parse_commands(const char *input, t_mini *mini);

// PIPING
void					open_close_pipe(t_parse_result *result,
							int *i, int (*pipe_fd)[2]);
...
void					print_all_child_errors(t_parse_result *res,
							int *exit_codes);
